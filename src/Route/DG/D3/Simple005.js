...
class Simple005 extends Component {
    ...
    renderD3 = () => {
        ...
        let x = d3.scaleTime().range([0, width]).domain(d3.extent(data, d => d.day)); // 设置mainChart x轴
        let x2 = d3.scaleTime().range([0, width]).domain(x.domain());  // 设置subChart x轴
        let y = d3.scaleLinear().rangeRound([height, 0]).domain([0, d3.max(data, d => d.quantity)]);// 设置mainChart y轴
